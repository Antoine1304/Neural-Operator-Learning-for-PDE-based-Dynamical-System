...
# --- Perceiver-style CrossAttention helper ---
class CrossAttentionBlock(nn.Module):
    """
    Cross-attention block: queries (Q) attend to keys/values (K,V).
    Uses MultiheadAttention for simplicity.
    Q: [B, S, Dq], K/V: [B, N, Dk]
    Returns: updated queries [B, S, Dout] (optionally residual + MLP)
    """
    def __init__(self, q_dim, kv_dim, out_dim, num_heads=8, dropout=0.0, mlp_hidden=None):
        super().__init__()
        ...
        mlp_hidden = mlp_hidden or (out_dim * 2)
    ...
    def forward(self, q, kv, kv_mask=None):
        ...
        k = self.k_proj(kv)
        v = self.v_proj(kv)
        # MultiheadAttention expects (B, L, D) with batch_first=True
        attn_out, _ = self.mha(q_in, k, v, key_padding_mask=kv_mask)  # [B,S,D]
        x = self.norm(q_in + attn_out)
        x = x + self.mlp(x)
        x = self.out_norm(x)
        return x
...
class GAOT(nn.Module):
    ...
    def __init__(self,
        ...
        # Perceiver seeds 
        self.use_perceiver_seeds = getattr(config.args.transformer, "use_perceiver_seeds", False)
        if self.use_perceiver_seeds:
            self.num_seeds = int(getattr(config.args.transformer, "num_seeds", 64))
            self.seed_dim = self.node_latent_size * (self.patch_size ** self.coord_dim)
            # Learnable seed embeddings
            self.seed_embeddings = nn.Parameter(torch.randn(self.num_seeds, self.seed_dim) * 0.02)
            # MLP to map seeds to working dimension
            self.seed_mlp = nn.Sequential(
                nn.Linear(self.seed_dim, self.seed_dim),
                nn.GELU(),
                nn.LayerNorm(self.seed_dim)
            )
            # Cross-attention for encoding and decoding
            self.cross_encode = CrossAttentionBlock(q_dim=self.seed_dim, kv_dim=self.seed_dim, out_dim=self.seed_dim,
                                                    num_heads=min(8, max(1, self.seed_dim // 16)))
            self.cross_decode = CrossAttentionBlock(q_dim=self.seed_dim, kv_dim=self.seed_dim, out_dim=self.seed_dim,
                                                    num_heads=min(8, max(1, self.seed_dim // 16)))

            # Fixed coordinate grid for seeds to enable Continuous Relative Bias
            S_side = int(self.num_seeds**0.5)
            if S_side * S_side == self.num_seeds:
                # Normalized 2D grid [0,1]
                grid_seeds = torch.stack(torch.meshgrid(
                    torch.linspace(0, 1, S_side), 
                    torch.linspace(0, 1, S_side), 
                    indexing='ij'
                ), dim=-1).reshape(-1, 2)
                self.register_buffer('seed_positions', grid_seeds)
            else:
                # Random fallback if num_seeds is not a perfect square
                self.register_buffer('seed_positions', torch.rand(self.num_seeds, 2))
    ...
    def process(self, rndata: Optional[torch.Tensor] = None,
        ...
        seq_len = rndata.shape[1]
        device = rndata.device

       # Determine relative_positions for the Transformer
        relative_positions = None

        if positions is not None:
            pos = positions.to(rndata.device)

            # Fixed mode: calculate patch centers from grid positions
            if pos.dim() == 2 and pos.shape[0] != seq_len:
                H, W = self.H, self.W
                P = self.patch_size
                num_patches_H = H // P
                num_patches_W = W // P

                # Reshape and average to find centers
                pos_grid = pos.view(H, W, -1)
                pos_grid = pos_grid.view(num_patches_H, P, num_patches_W, P, -1)
                patch_centers = pos_grid.mean(dim=(1, 3))
                relative_positions = patch_centers.view(-1, pos.shape[1])
            else:
                # Use provided positions directly
                relative_positions = pos
        else:
            # Fallback to default positional embeddings
        ...
        # Perceiver seed logic: project tokens to smaller set of latent seeds
        if self.use_perceiver_seeds and seq_len > self.num_seeds:
            B = rndata.shape[0]
            tokens = rndata

            # Initialize and condition seed embeddings
            seeds = self.seed_embeddings.unsqueeze(0).expand(B, -1, -1).to(device)
            seeds = self.seed_mlp(seeds)

            # Encode: seeds query tokens
            seeds = self.cross_encode(seeds, tokens)

            # Handle projection to processor dimension if necessary
            proc_in_dim = self.processor.input_proj.in_features if hasattr(self.processor, "input_proj") and isinstance(self.processor.input_proj, nn.Linear) else seeds.shape[-1]
            if seeds.shape[-1] != proc_in_dim:
                if not hasattr(self, "_seed_to_proc"):
                    self._seed_to_proc = nn.Linear(seeds.shape[-1], proc_in_dim).to(device)
                    self._proc_to_seed = nn.Linear(proc_in_dim, seeds.shape[-1]).to(device)
                proc_in = self._seed_to_proc(seeds)
            else:
                proc_in = seeds

            # Process latent seeds with Transformer using seed positions
            batch_seed_pos = self.seed_positions.unsqueeze(0).expand(B, -1, -1)
            seeds_proc = self.processor(proc_in, condition=condition, relative_positions=batch_seed_pos)

            # Map back to feature dimension
            if hasattr(self, "_proc_to_seed"):
                seeds_proc = self._proc_to_seed(seeds_proc)

            # Decode: tokens query processed seeds
            rndata = self.cross_decode(tokens, seeds_proc)
        else:
            # Default path: direct processing of tokens
            rndata = self.processor(rndata, condition=condition, relative_positions=relative_positions)
        
        # Reshape output back to regional node format
        if self.coord_dim == 2:
            rndata = rndata.view(batch_size, num_patches_H, num_patches_W, P, P, C)
            rndata = rndata.permute(0, 1, 3, 2, 4, 5).contiguous()
            rndata = rndata.view(batch_size, H * W, C)
