...
class AttentionConfig:
    ...
    use_continuous_relative_bias: bool = False  # Whether to use continuous relative bias
    init_relative_scale: float = 1.0        # Initial scale for the continuous relative
...
class TransformerConfig:
    ...
    use_perceiver_seeds: bool = False
    num_seeds: int = 256
    seed_hidden_size: int = 128   # optional intermediate size for seed MLP
...
class GroupQueryFlashAttention(nn.Module):
    def __init__(self, 
                 ...
                 use_continuous_relative_bias: bool = False,
                 init_relative_scale: float = 1.0
                 ):
        super().__init__()
        ...
        # --- Continuous relative bias option (simple RBF-like param per head) ---
        self.use_continuous_relative_bias = use_continuous_relative_bias
        if self.use_continuous_relative_bias:
            # learnable scale per head (positive by construction via softplus)
            self._rel_scale = nn.Parameter(torch.ones(self.num_heads) * float(init_relative_scale))
            # small clamp to avoid extreme values
            self._rel_scale_clamp = (1e-6, 1e3)
    ...
    def forward(self, x, condition: Optional[float] = None, relative_positions: Optional[torch.Tensor] = None):
        ...
        relative_positions: optional coordinates used to compute continuous relative bias.
            - If provided as [seq_len, coord_dim] (fx), same coords used for all batch elements.
            - If provided as [batch_size, seq_len, coord_dim] (vx), per-batch coords supported.
        """
        ...
        k = k.view(batch_size, seq_len, self.num_kv_heads, self.head_dim).transpose(1, 2)  # (B, H_kv, L, D)
        ...
        if self.rotary_emb is not None and relative_positions is not None:
            q = self.rotary_emb.rotate_queries_or_keys(q)
            k = self.rotary_emb.rotate_queries_or_keys(k)

        # Compute attention with continuous relative bias if requested
        if self.use_continuous_relative_bias and (relative_positions is not None):
            # Calculate pairwise distances
            pos = relative_positions
            d = torch.cdist(pos, pos)
            d = d.unsqueeze(0).expand(batch_size, -1, -1)

            # Compute RBF-style bias scaled per head
            rel_scale = torch.nn.functional.softplus(
                self._rel_scale.clamp(min=self._rel_scale_clamp[0], max=self._rel_scale_clamp[1])
            ).view(1, self.num_heads, 1, 1)
            
            bias = - (d.unsqueeze(1) ** 2) * rel_scale

            # compute scaled dot-product logits manually
            ...
            if dp > 0.0:
                attn = F.dropout(attn, p=dp, training=self.training)

            # attention output
            out = torch.einsum('bhij,bhjd->bhid', attn, v)  # [B,H,L,D]
            x = out.transpose(1, 2).contiguous().view(batch_size, seq_len, -1)
            x = self.o_proj(x)
            return x
    ...
    @classmethod
    def from_config(cls, input_size: int, hidden_size: int, config: AttentionConfig, positional_embedding: str = "absolute", **extra):
        """
        Create GroupQueryFlashAttention from an AttentionConfig.
        Accepts extra keyword args (backwards compatible) and forwards them to the constructor.
        """
        cfg_dict = shallow_asdict(config)
        # merge config dict with any extra kwargs (extra overrides cfg_dict)
